 ...
 * @author Waseem Anjum
 */
public class F_Envy extends ASTVisitor{
    int low_ins;
    int low_m;
    int ex_fields=0;
    int total_eny=0;
    ...
    public boolean visit(MethodDeclaration node)
    {
          // this_list.add(node.getName().toString());
            ...
            for(int j=0;j<list_all_methods.size();j++)
            {
             ...
             if(other_methods.size()>=low_m && other_ins.size()>=low_ins)
            {
                //JOptionPane.showMessageDialog(null, "chal rya yar");
                String str1="\n>>>>Foreign Instances<<<< \n";
                String str2="\n>>>>Foreign Methods<<<< \n";
                ...
                try {
                    ...
                    Logger.getLogger(F_Envy.class.getName()).log(Level.SEVERE, null, ex);
                }
                temp[2] = this_method;
                temp[3] = ""+other_ins.size();
                temp[4] = ""+other_methods.size();
    ...
    public  ArrayList<String[]> get_total_envy()
    {
